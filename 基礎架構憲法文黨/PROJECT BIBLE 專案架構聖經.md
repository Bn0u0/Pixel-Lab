...
all objects must adhere to the following specifications:
...
由 /data/config.js 中的 PIXEL_SCALE (預設 4x) 全局變量控制。
...
純深色: 無光影計算。用於強調哥德風的輪廓，確保角色在複雜背景中依然清晰可見。
...
次表面散射: 基礎色 + 邊緣泛紅。在渲染時，偵測該像素是否鄰近透明區域，若是，則疊加 20% 紅色透明度，模擬皮膚透光感。
...
程序化噪點: 基礎色 + 雜訊。使用 (x, y) 座標作為種子 (Seed) 生成穩定的噪點，避免動畫時閃爍。用於增加細節密度。
...
頂光漸層: 上亮下暗。根據像素在 Sprite 中的相對 Y 座標，疊加線性漸層 (Linear Gradient)，模擬布料的自然垂墜感。
...
動態高光: 鏡面反射。根據全域 time 變量，產生一條掃過表面的高亮白線，模擬金屬光澤。
...
啞光質感: 僅在頂部 2px 產生微弱高光，其餘部分保持低對比度，模擬皮革的吸光特性。
