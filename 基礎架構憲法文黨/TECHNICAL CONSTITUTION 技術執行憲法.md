...
- **容器尺寸 (Canvas Size)**: `32x48` (統一規格).
    - **理由**: 為了容納最高的人類敵人 (48px) 並預留史萊姆進化後的成長空間。
    - **史萊姆本體**: 初始狀態應只佔用底部 `32x24` 區域。除非進化，否則不可填滿畫布，以維持「弱小 vs 強大」的視覺階級。
- **碰撞箱 (Hitbox)**: **動態 (Dynamic)**.
    - 程式需支援 `width/height` 與 `sprite` 尺寸解耦。
    - 史萊姆初始 Hitbox: `24x24` (Centered at bottom).
